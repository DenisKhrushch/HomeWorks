...
let getPizzasByComposition = composition => {
    ...
    for (let i = 0; i < pizzaList.length; i++) {
        for (let j = 0; j < pizzaList[i].composition.length; j++) {
            if (pizzaList[i].composition[j] === composition) {
                tempArr.push(pizzaList[i]);
            }
        }
    }
...
const getAllWithoutPizzaById = id => {
    let removed = pizzaList.splice(id - 1,1);
    console.log(removed);
};

getAllWithoutPizzaById(2);
